...
## WebSite: http://ahmad.jempress.com, https://gallery.technet.microsoft.com/scriptcenter/Format-Table-Colors-in-e0a4beac
...
Function Write-PSObject
{
    ...
    #region Defaults
    If ($HeadersForeColor -eq -1)
    {
        $HeadersForeColor = "White";
    }
    
    If ($BodyForeColor -eq -1)
    {
        $BodyForeColor = "White";
    }

    If ($HeadersBackColor -eq -1)
    {
        $HeadersBackColor = "DarkMagenta";
    }
    
    If ($BodyBackColor -eq -1)
    {
        $BodyBackColor = "DarkMagenta";
    }
    #endregion Defaults

    #region Format Table 
    If ($OddRowForeColor -eq -1)
    {
        $OddRowForeColor = "White";
    }
    
    If ($EvenRowForeColor -eq -1)
    {
        $EvenRowForeColor = "White";
    }

    If ($OddRowBackColor -eq -1)
    {
        $OddRowBackColor = "DarkMagenta";
    }
    
    If ($EvenRowBackColor -eq -1)
    {
        $EvenRowBackColor = "DarkMagenta";
    }
    ...
    If (!($ValueForeColor))
    {
        #$ValueForeColor = @($BodyForeColor) * $Value.Count;
    }

    If($ValueBackColor.Count -eq 0)
    {
        #$ValueBackColor = @($BodyBackColor) * $Value.Count;
    }
    ...
    Foreach($line in $lines) 
    {
        ...
        If($l -le 2)
        {
            If($l -eq 2 -And ($MatchMethod -Or $ColoredColumns))
            {
                ...
                $columns = @($null) * $headerLineCount;
                
                [Int[]] $headersPos = @(0) * $headerLineCount;
                [Int[]] $headersLen = @(0) * $headerLineCount;
                ...
                While($pos -ne -1)
                {
                    ...
                    $headersPos[$i] = $pos;
                    $headersLen[$i - 1] = $pos - $headersPos[$i - 1];
            ...
            If(!($BodyOnly))
            {
                ...
                Write-Host -Object $line -ForegroundColor $hfc -BackgroundColor $hbc;
            }
        ...
        {
            $values = @($null) * $headerLineCount;

            For($i = 0; $i -lt $headerLineCount; $i++)
            {
                If ($i -eq ($headerLineCount -1))
                {
                    $values[$i] = $line.Substring($headersPos[$i]).Trim();
                }
                else
                {
                    $values[$i] = $line.Substring($headersPos[$i], $headersLen[$i]).Trim();
                }
            }
        }
        ...
        $fc = @($BodyForeColor) * $headerLineCount;
        $bc = @($BodyBackColor) * $headerLineCount;
        ...
        If($MatchMethod -Or $ColoredColumns)
        {
            ...
            For($j = 0; $j -lt $columns.Count; $j++)
            {
                ...
                else
                {
                    ...
                    {
                        Write-Host " " -NoNewline -BackgroundColor $RowBackColor;
                    }
                    else
                    {
                        Write-Host " " -NoNewline -BackgroundColor $BodyBackColor;
                    }
                ...
                Write-Host -Object $valueText -NoNewline -ForegroundColor $foreColor -BackgroundColor $backColor;
        ...
        ElseIf(!($HeadersOnly))
        {
            Write-Host -ForegroundColor $BodyForeColor -BackgroundColor $BodyBackColor $line;
        }
        ...
        If ($l -ne ($lines.Length))
        {
            If ($InjectRowsSeparator)
            {
                ...
                Write-Host -Object $RowsSeparatorLine -ForegroundColor $RowsSeparatorForeColor -BackgroundColor $RowsSeparatorBackColor;
            }
        }
    }
}